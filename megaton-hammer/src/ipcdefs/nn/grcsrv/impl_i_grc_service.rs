...
impl IGrcService<Session> {
	pub fn raw_new() -> Result<IGrcService<Session>> {
		...
		let session = sm.get_service(*b"grc:c\0\0\0")?;
	...
	pub fn new() -> Result<Arc<IGrcService<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IGrcService<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"grc:c\0\0\0") {
