...
impl IServiceManager<Session> {
	pub fn raw_new() -> Result<IServiceManager<Session>> {
		use ::ipcdefs::nn::sm::detail::IUserInterface;

		let sm = IUserInterface::raw_new()?;

		let session = sm.get_service(*b"htc:tenv")?;
	...
	pub fn new() -> Result<Arc<IServiceManager<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IServiceManager<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"htc:tenv") {
			let ret = Arc::new(IServiceManager(ManuallyDrop::into_inner(hnd)));
