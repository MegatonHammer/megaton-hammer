...
impl IProgramRegistry<Session> {
	pub fn raw_new() -> Result<IProgramRegistry<Session>> {
		...
		let session = sm.get_service(*b"fsp-pr\0\0")?;
	...
	pub fn new() -> Result<Arc<IProgramRegistry<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IProgramRegistry<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"fsp-pr\0\0") {
