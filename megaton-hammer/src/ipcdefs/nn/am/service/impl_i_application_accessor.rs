...
impl<T: Object> IApplicationAccessor<T> {
	...
	pub fn get_current_library_applet(&self, ) -> Result<::ipcdefs::nn::am::service::IAppletAccessor<T>> {
		use ::ipc::{Request, Response};
	...
	pub fn get_application_id(&self, ) -> Result<::ipcdefs::nn::ncm::ApplicationId> {
		use ::ipc::{Request, Response};

		let req : Request<_, [_; 0], [_; 0], [_; 0]> = Request::new(120)
			.args(())
			;
		let res : Response<::ipcdefs::nn::ncm::ApplicationId> = self.0.send(req)?;
		Ok(*res.get_raw())
	}

	pub fn push_launch_parameter(&self, unk0: u32, unk1: &::ipcdefs::nn::am::service::IStorage<Session>) -> Result<()> {
		use ::ipc::{Request, Response};
