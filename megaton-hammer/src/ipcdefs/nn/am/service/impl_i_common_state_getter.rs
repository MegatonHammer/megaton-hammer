...
impl<T: Object> ICommonStateGetter<T> {
	...
	pub fn receive_message(&self, ) -> Result<::ipcdefs::nn::am::AppletMessage> {
		use ::ipc::{Request, Response};

		let req : Request<_, [_; 0], [_; 0], [_; 0]> = Request::new(1)
			.args(())
			;
		let res : Response<::ipcdefs::nn::am::AppletMessage> = self.0.send(req)?;
		Ok(*res.get_raw())
	}

	pub fn get_this_applet_kind(&self, ) -> Result<::ipcdefs::nn::am::service::AppletKind> {
		use ::ipc::{Request, Response};

		let req : Request<_, [_; 0], [_; 0], [_; 0]> = Request::new(2)
			.args(())
			;
		let res : Response<::ipcdefs::nn::am::service::AppletKind> = self.0.send(req)?;
		Ok(*res.get_raw())
	}
	...
	pub fn push_to_general_channel(&self, unk0: &::ipcdefs::nn::am::service::IStorage<Session>) -> Result<()> {
		use ::ipc::{Request, Response};

		let req : Request<_, [_; 0], [_; 1], [_; 0]> = Request::new(20)
			.args(())
	...
	pub fn get_home_button_reader_lock_accessor(&self, ) -> Result<::ipcdefs::nn::am::service::ILockAccessor<T>> {
		use ::ipc::{Request, Response};
	...
	pub fn get_reader_lock_accessor_ex(&self, unk0: i32) -> Result<::ipcdefs::nn::am::service::ILockAccessor<T>> {
		use ::ipc::{Request, Response};
