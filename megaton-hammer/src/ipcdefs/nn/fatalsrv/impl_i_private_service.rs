...
impl IPrivateService<Session> {
	pub fn raw_new() -> Result<IPrivateService<Session>> {
		...
		let session = sm.get_service(*b"fatal:p\0")?;
	...
	pub fn new() -> Result<Arc<IPrivateService<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IPrivateService<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"fatal:p\0") {
