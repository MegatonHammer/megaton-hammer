...
impl IDsService<Session> {
	...
	pub fn new() -> Result<Arc<IDsService<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IDsService<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"usb:ds\0\0") {
...
impl<T: Object> IDsService<T> {
	...
	pub fn set_vid_pid_bcd(&self, descriptor: &::ipcdefs::nn::usb::UsbDescriptorData) -> Result<()> {
		use ::ipc::IPCBuffer;
		use ::ipc::{Request, Response};
