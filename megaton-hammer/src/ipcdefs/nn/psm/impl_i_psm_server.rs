...
impl IPsmServer<Session> {
	pub fn raw_new() -> Result<IPsmServer<Session>> {
		...
		let session = sm.get_service(*b"psm\0\0\0\0\0")?;
	...
	pub fn new() -> Result<Arc<IPsmServer<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IPsmServer<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"psm\0\0\0\0\0") {
