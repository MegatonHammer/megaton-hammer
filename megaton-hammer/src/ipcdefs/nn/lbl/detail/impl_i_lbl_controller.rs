...
impl ILblController<Session> {
	pub fn raw_new() -> Result<ILblController<Session>> {
		...
		let session = sm.get_service(*b"lbl\0\0\0\0\0")?;
	...
	pub fn new() -> Result<Arc<ILblController<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<ILblController<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"lbl\0\0\0\0\0") {
