...
impl LocationResolverInterface<Session> {
	pub fn raw_new() -> Result<LocationResolverInterface<Session>> {
		...
		let session = sm.get_service(*b"lr\0\0\0\0\0\0")?;
	...
	pub fn new() -> Result<Arc<LocationResolverInterface<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<LocationResolverInterface<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"lr\0\0\0\0\0\0") {
