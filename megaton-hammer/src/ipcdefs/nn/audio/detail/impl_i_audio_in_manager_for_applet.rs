...
impl IAudioInManagerForApplet<Session> {
	pub fn raw_new() -> Result<IAudioInManagerForApplet<Session>> {
		...
		let session = sm.get_service(*b"audin:a\0")?;
	...
	pub fn new() -> Result<Arc<IAudioInManagerForApplet<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IAudioInManagerForApplet<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"audin:a\0") {
