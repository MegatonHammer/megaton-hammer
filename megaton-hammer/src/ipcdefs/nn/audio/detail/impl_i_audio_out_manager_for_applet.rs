...
impl IAudioOutManagerForApplet<Session> {
	...
	pub fn new() -> Result<Arc<IAudioOutManagerForApplet<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IAudioOutManagerForApplet<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"audout:a") {
