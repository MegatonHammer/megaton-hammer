...
impl IGeneralInterface<Session> {
	...
	pub fn new() -> Result<Arc<IGeneralInterface<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IGeneralInterface<Session>>> = InternalMutex::new(Weak::new());
		}
...
impl<T: Object> IGeneralInterface<T> {
	...
	// fn import_ssl_rsa_key(&self, UNKNOWN) -> Result<UNKNOWN>;
	// fn secure_exp_mod_with_ssl_key(&self, UNKNOWN) -> Result<UNKNOWN>;
	// fn import_es_rsa_key(&self, UNKNOWN) -> Result<UNKNOWN>;
	// fn secure_exp_mod_with_es_key(&self, UNKNOWN) -> Result<UNKNOWN>;
	// fn encrypt_manu_rsa_key_for_import(&self, UNKNOWN) -> Result<UNKNOWN>;
	// fn get_package2_hash(&self, UNKNOWN) -> Result<UNKNOWN>;
}
