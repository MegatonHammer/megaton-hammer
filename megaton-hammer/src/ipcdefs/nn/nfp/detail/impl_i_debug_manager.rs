...
impl IDebugManager<Session> {
	...
	pub fn new() -> Result<Arc<IDebugManager<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : InternalMutex<Weak<IDebugManager<Session>>> = InternalMutex::new(Weak::new());
		}
		if let Some(hnd) = HANDLE.lock().upgrade() {
			return Ok(hnd)
		}

		if let Some(hnd) = ::loader::get_override_service(*b"nfp:dbg\0") {
...
impl<T: Object> IDebugManager<T> {
	pub fn create_debug_interface(&self, ) -> Result<T> {
