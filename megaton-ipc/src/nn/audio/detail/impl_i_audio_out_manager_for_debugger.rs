...
impl IAudioOutManagerForDebugger<Session> {
	pub fn new() -> Result<Arc<IAudioOutManagerForDebugger<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IAudioOutManagerForDebugger<Session>>> = Mutex::new(Weak::new());
		}
		...
		if let Some(hnd) = ::megaton_hammer::loader::get_override_service(*b"audout:d") {
		...
		let r = sm.get_service(*b"audout:d").map(|s: KObject| Arc::new(Session::from(s).into()));
		if let Ok(service) = r {
