...
impl IHidServer {
	pub fn new() -> Result<Arc<IHidServer>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		use core::mem::ManuallyDrop;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IHidServer>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"hid\0\0\0\0\0").map(|s| Arc::new(unsafe { IHidServer::from_kobject(s) }));
		if let Ok(service) = r {
...
impl IHidServer {
	pub fn create_applet_resource(&self, unk0: ::nn::applet::AppletResourceUserId) -> Result<::nn::hid::IAppletResource> {
	...
	pub fn create_active_vibration_device_list(&self, ) -> Result<::nn::hid::IActiveVibrationDeviceList> {
		use megaton_hammer::ipc::{Request, Response};
