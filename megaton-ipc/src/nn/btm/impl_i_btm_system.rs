...
impl IBtmSystem {
	pub fn new() -> Result<Arc<IBtmSystem>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		use core::mem::ManuallyDrop;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IBtmSystem>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"btm:sys\0").map(|s| Arc::new(unsafe { IBtmSystem::from_kobject(s) }));
		if let Ok(service) = r {
