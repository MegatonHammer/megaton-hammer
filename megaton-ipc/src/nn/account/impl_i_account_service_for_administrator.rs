...
impl IAccountServiceForAdministrator {
	pub fn new() -> Result<Arc<IAccountServiceForAdministrator>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IAccountServiceForAdministrator>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"acc:su\0\0").map(|s| Arc::new(unsafe { IAccountServiceForAdministrator::from_kobject(s) }));
		if let Ok(service) = r {
