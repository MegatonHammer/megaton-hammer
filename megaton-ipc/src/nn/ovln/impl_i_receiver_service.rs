...
impl IReceiverService<Session> {
	pub fn new() -> Result<Arc<IReceiverService<Session>>> {
		...
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IReceiverService<Session>>> = Mutex::new(Weak::new());
		}
		...
		if let Some(hnd) = ::megaton_hammer::loader::get_override_service(*b"ovln:rcv") {
		...
		let r = sm.get_service(*b"ovln:rcv").map(|s: KObject| Arc::new(Session::from(s).into()));
		if let Ok(service) = r {
