...
impl IManager<Session> {
	pub fn new() -> Result<Arc<IManager<Session>>> {
		...
		if let Some(hnd) = ::megaton_hammer::loader::get_override_service(*b"pwm\0\0\0\0\0") {
		...
		let r = sm.get_service(*b"pwm\0\0\0\0\0").map(|s: KObject| Arc::new(Session::from(s).into()));
		if let Ok(service) = r {
...
impl<T: Object> IManager<T> {
	pub fn open_session_for_dev(&self, unk0: u32) -> Result<T> {
