...
impl ISystemManager {
	pub fn new() -> Result<Arc<ISystemManager>> {
		...
		if let Some(hnd) = ::megaton_hammer::loader::get_override_service(*b"nfp:sys\0") {
		...
		let r = sm.get_service(*b"nfp:sys\0").map(|s| Arc::new(unsafe { ISystemManager::from_kobject(s) }));
		if let Ok(service) = r {
