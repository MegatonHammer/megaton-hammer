...
impl IDebugManager {
	pub fn new() -> Result<Arc<IDebugManager>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IDebugManager>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"nfp:dbg\0").map(|s| Arc::new(unsafe { IDebugManager::from_kobject(s) }));
		if let Ok(service) = r {
