...
impl IEthInterfaceGroup {
	pub fn new() -> Result<Arc<IEthInterfaceGroup>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IEthInterfaceGroup>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"ethc:i\0\0").map(|s| Arc::new(unsafe { IEthInterfaceGroup::from_kobject(s) }));
		if let Ok(service) = r {
