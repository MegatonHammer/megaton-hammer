...
impl IEthInterface {
	pub fn new() -> Result<Arc<IEthInterface>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<IEthInterface>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"ethc:c\0\0").map(|s| Arc::new(unsafe { IEthInterface::from_kobject(s) }));
		if let Ok(service) = r {
