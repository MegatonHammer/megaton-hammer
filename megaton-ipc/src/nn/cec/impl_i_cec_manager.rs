...
impl ICecManager {
	pub fn new() -> Result<Arc<ICecManager>> {
		use alloc::arc::Weak;
		use spin::Mutex;
		lazy_static! {
			static ref HANDLE : Mutex<Weak<ICecManager>> = Mutex::new(Weak::new());
		}
		...
		let r = sm.get_service(*b"cec-mgr\0").map(|s| Arc::new(unsafe { ICecManager::from_kobject(s) }));
		if let Ok(service) = r {
